...
// FIXED: Updated createCommunity function
export const createCommunity = async (communityData) => {
  try {
    ...
    const { data: membership, error: membershipError } = await supabase
      .from("community_memberships")
      .insert({
        user_id: communityData.leader_id,
    ...
    if (membershipError) {
      ...
      // If membership creation fails, we should clean up the community
    ...
    // Return the community data with success indicator
...
export const addReputationPoints = async (
  userId,
  points,
  reason,
  ...
  const { data, error } = await supabase
    .from("profiles")
    .update({
      reputation_points: supabase.raw(`reputation_points + ${points}`),
    })
    .eq("id", userId)
    .select()
    .single();
